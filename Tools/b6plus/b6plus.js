 ...
 *
 * TODO: When a video is playing in both the first and second windows,
 * should we mute one of the two, or leave it to the user? And which
 * one would we mute?
 *
...
const translations = {
  ...
  "pause/resume automatic slide show": {
    ...
    fr: "pause/reprise du diapos automatiques",
...
function numberSlides()
{
  ...
  for (const h of document.body.children)
    if (isStartOfSlide(h)) {
      ...
      for (const v of h.getElementsByTagName('VIDEO'))
...
/* instrumentVideos -- add event handlers to all video elements */
function instrumentVideos()
{
  /* If a second window is open, these event handlers help to
   * synchronize the playback of videos in both windows. When starting
   * a video in one window, a message is sent to the other window to
   * start the video there, too. See message() for how the message is
   * handled in the receiving window. */
...
function keyDown(event)
{
  ...
  case 'p':
  case 'MediaPlay':
    /* Note that calling displaySlide() and resets the time. */
    if (syncmode) warnSyncMode()
...
function finishToggleMode()
{
  if (stylesToLoad != 0 && Date.now() < limit) {
    ...
    /* There may be tall objects. Make sure we show the start of the slide. */
...
function nextSlide()
{
  ...
  else if (loopSlideShow) firstSlide();
}
...
function message(e)
{
  ...
  if (e.source == secondwindow) { // Message from 2nd window to 1st window
    ...
    case 'pause':		// Second window tells us a video was paused
      document.getElementById(e.data.id)?.pause();
      break;
    case 'play':		// Second window tells us a video was started
      document.getElementById(e.data.id)?.play();
      break;
    ...
    case 'pause':		// Second window tells us a video was paused
      document.getElementById(e.data.id)?.pause();
      break;
    case 'play':		// Second window tells us a video started
      document.getElementById(e.data.id)?.play();
      break;
    case 'seeked':		// Second window tells us a video was seeked
      if ((h = document.getElementById(e.data.id))) {
